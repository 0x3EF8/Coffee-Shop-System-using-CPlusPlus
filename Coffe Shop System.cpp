...
int main(){
	...
	head = NULL;
	insertfirst(1,"Americano",23,2.23);
	insertend(2,"Latte    ",13,1.67);
	insertend(3,"Cappuccino",8,3.83);
	insertend(4,"Espresso  ",46,5.23);
	insertend(5,"Irish     ",46,5.23);
	insertend(6,"Nescafe  ",34,5.23);
	insertend(7,"Creamy Latte",7,5.29);
	insertend(8,"Creamy White",121,6.13);
	insertend(9,"Flat White",73,7.13);
	...
	if(get_food_choice == temp->data){
		...
		if(confirm == 1 ){
			...
			if(payment==1){
				...
				br(2);pre(4);  printf("1. Buy Another Coffe ? ");
